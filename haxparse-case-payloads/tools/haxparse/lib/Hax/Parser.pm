...
sub parse_pattern ($self) {
  ...
  if ($self->_peek_p('(')) {
    $self->_eat_p('(');
    if (!$self->_peek_p(')')) {
      push @binds, $self->parse_pat_bind;
      while ($self->_peek_p(',')) {
        $self->_eat_p(',');
        push @binds, $self->parse_pat_bind;
      }
  ...
  # Back-compat: older tooling expected a single {bind}.
  my $bind = (@binds == 1) ? $binds[0] : undef;

  return node('PatternVariant', span($t0, $t0), name => $variant, binds => \@binds, bind => $bind);
}

sub parse_pat_bind ($self) {
  my $t0 = $self->{cur};
  my $type;
  ...
  my ($sigil, $name) = $self->parse_var_name;
  return node('PatBind', span($t0, $self->{cur}), type => $type, sigil => $sigil, name => $name);
}
